...
The `ml-framework-snapshots` tool verifies that `zero-chex` accurately replicates the public API signatures, parameters, and defaults of the real `chex` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check chex ~/repos/zero-chex/src \
    --reference-prefix chex \
    --target-prefix zero_chex
```

**What it does:**
- It statically analyzes all Python modules in `zero-chex/src`.
- It maps the target namespace (`zero_chex.*`) back to the reference framework (`chex.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-chex/src
```
