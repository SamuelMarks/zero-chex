...
def main():
    ...
    for mod_name, funcs in modules.items():
        if not funcs:
            continue
        prefix = "CHEX_TODO.md".split("_")[0].lower()
        orig_mod_name = mod_name.replace("zero_chex", prefix)
        mod_implemented = sum(
    ...
    if new_todo != current_todo:
        with open("CHEX_TODO.md", "w") as f:
            f.write(new_todo)
        print("Updated CHEX_TODO.md.")
